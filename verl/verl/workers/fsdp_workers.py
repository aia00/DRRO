...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    async def rollout_mode(self):
        ...
        if self.config.rollout.name == "vllm" and isinstance(params, dict):
            # vLLM expects model.embed_tokens.weight; drop or rename embed_tokens.weight.
            if "embed_tokens.weight" in params:
                if "model.embed_tokens.weight" not in params:
                    params["model.embed_tokens.weight"] = params["embed_tokens.weight"]
                params.pop("embed_tokens.weight", None)
        ...
        if peft_config is not None and getattr(self.rollout, "sleep_level", None) == 2:
            ...
            if self.config.rollout.name == "vllm" and isinstance(base_model_params, dict):
                if "embed_tokens.weight" in base_model_params:
                    if "model.embed_tokens.weight" not in base_model_params:
                        base_model_params["model.embed_tokens.weight"] = base_model_params["embed_tokens.weight"]
                    base_model_params.pop("embed_tokens.weight", None)
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        timing_generate = {}
        use_async_rollout = self.config.rollout.mode == "async"
        if self._is_actor and not use_async_rollout:  # For rollout only, we do not switch context.
            loop = get_event_loop()
        ...
        if self._is_actor and not use_async_rollout:
